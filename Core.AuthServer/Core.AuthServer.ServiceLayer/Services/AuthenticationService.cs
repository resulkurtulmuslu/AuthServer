...
namespace Core.AuthServer.ServiceLayer.Services
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<Response<TokenDto>> CreateTokenAsync(LoginDto loginDto)
        {
            ...
            if (user == null) return Response<TokenDto>.Fail("Email veya password yalnış.", 400, true);

            if (!await _userManager.CheckPasswordAsync(user, loginDto.Password))
                return Response<TokenDto>.Fail("Email veya password yalnış.", 400, true);

            var token = _tokenService.CreateToken(user);

            var userRefreshToken = await _userRefreshTokenService.Where(x => x.UserId == user.Id)
        ...
        public Response<ClientTokenDto> CreateTokenByClient(ClientLoginDto clientLoginDto)
        {
            ...
            x.Id == clientLoginDto.ClientId
            && x.Secret == clientLoginDto.ClientSecret);

            if (client == null) return Response<ClientTokenDto>
        ...
        public async Task<Response<TokenDto>> CreateTokenByRefreshToken(string refreshToken)
        {
            ...
            if (existRefreshToken == null) return Response<TokenDto>
                     .Fail("Refresh token not found", 404, true);

            var user = await _userManager.FindByIdAsync(existRefreshToken.UserId);

            if (user == null) return Response<TokenDto>
                    .Fail("UserId not found", 404, true);
        ...
        public async Task<Response<NoDataDto>> RevokeRefreshToken(string refreshToken)
        {
            ...
            if (existRefreshToken == null) return Response<NoDataDto>
                    .Fail("Refreh token not found", 404, true);
